...
public class Escopo {
	...
	Cliente c1;
	DaoCliente sql;
	static Escopo window = new Escopo();
	...
	private void initialize() {
		...
		vSolucao_NxtOperations.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(vSolucao_NxtOperations.isSelected()) {
					...
					//Os produtos sç¸Š desselecionados
		...
		btnNewButton_Prosseguir.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				//Adicionando na array de produtos as opé‹�es de NxtDemand
				...
				//Adicionando na array de produtos as opé‹�es de Operations
				...
				abc = vProList;
				// Mostrar as informaé‹�es da ativaè»‹o na tela
				
				
				//lista para armazenar vç–µias pessoas
				
				//se houver dados para prosseguir
				//armazenando a soluéˆ¬o selecionado
				...
				if(!textField_vCliente.getText().equals("") &&
						...
						vProList.size() != 0)
				{
					// um objeto pessoa ï¿½ criado
					c1=new Cliente(textField_vCliente.getText());
					
					//salvando a soluéˆ¬o selecionada no atributo do cliente
					if(vSolucao_NxtDemand.isSelected()) {
					...
					//Criando as opé‹�es do alerta
					Object[] options = { "Confirmar", "Cancelar" };
					int retorno=JOptionPane.showOptionDialog(null, "Cliente: "+c1.getvCliente()+"\n"+
															"Soluéˆ¬o: "+c1.getvSolucao()+"\n"+
															"Objetivo: "+c1.getvObjetivo()+"\n"+
															"Entregas Minimas: "+c1.getvEntregaM()+"\n"+
															"Entregas Possiveis: "+c1.getvEntregaP()+"\n"+
															"Produtos Selecionados:"+vProList+"\n"+
															"Vocï¿½ deseja continuar: ", "Informaéˆ¬o", JOptionPane.DEFAULT_OPTION, JOptionPane.QUESTION_MESSAGE, null, options, options[0]);
					
					//Definindo as aé‹�es do alerta
					
					//Se o usä¿‰rio confirma segue para a proxima etap
					if(retorno == 0){
						...
						/*
						 * DaoSolucao daoSolucao = new DaoSolucao(); daoSolucao.novaSol(c1);
						 */
						
						/*DaoCliente_Solucao daoCS = new DaoCliente_Solucao();
						daoCS.novaCS(c1);*/
						
						/*
						 * DaoProduto daoPR = new DaoProduto(window); daoPR.novaPR(c1);
						 */
				...
				//Se houver um campo vazio o programa nç¸Š continua
				else {
